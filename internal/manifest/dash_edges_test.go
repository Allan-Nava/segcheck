...
func TestExpandTemplate_OpenEndedRepeat(t *testing.T) {
	...
	_, segs, err := expandTemplate(tmpl, mpdRepresentation{ID: "v"}, base, time.Time{}, epoch, 0, 10, false, false)
	if err != nil {
	...
	_, segs, err = expandTemplate(tmpl, mpdRepresentation{ID: "v"}, base, time.Time{}, epoch, 0, 0, false, false)
	if err != nil {
		t.Fatalf("expandTemplate: %v", err)
	}
	if len(segs) != 1 {
...
func TestExpandTemplate_OpenEndedRepeatShorterThanOneSegment(t *testing.T) {
	...
	base, _ := url.Parse("https://cdn.example.com/d/m.mpd")
	_, segs, err := expandTemplate(tmpl, mpdRepresentation{ID: "v"}, base, time.Time{}, epoch, 0, 2, false, false)
	if err != nil {
		t.Fatalf("expandTemplate: %v", err)
	}
	if len(segs) != 1 {
...
func TestExpandTemplate_BoundsTheSegmentListFromADuration(t *testing.T) {
	...
	_, segs, err := expandTemplate(tmpl, mpdRepresentation{}, base, time.Time{}, epoch, 0, maxExpandedSegments+5000, false, false)
	if err != nil {
		t.Fatalf("expandTemplate: %v", err)
	}
	if len(segs) != maxExpandedSegments {
...
func TestExpandTemplate_NeitherTimelineNorDuration(t *testing.T) {
	...
	base, _ := url.Parse("https://cdn.example.com/d/m.mpd")
	if _, _, err := expandTemplate(tmpl, mpdRepresentation{}, base, time.Time{}, epoch, 0, 10, false, false); err == nil {
...
func TestExpandTemplate_StaticWithoutAPresentationDuration(t *testing.T) {
	...
	base, _ := url.Parse("https://cdn.example.com/d/m.mpd")
	if _, _, err := expandTemplate(tmpl, mpdRepresentation{}, base, time.Time{}, epoch, 0, 0, false, false); err == nil {
...
func TestExpandTemplate_LiveBeforeAnythingIsAvailable(t *testing.T) {
	...
	_, _, err := expandTemplate(tmpl, mpdRepresentation{}, base, ast, epoch, 0, 0, true, false)
	if err == nil {
...
func TestExpandTemplate_LiveWindowShorterThanTheSampleWindow(t *testing.T) {
	...
	_, segs, err := expandTemplate(tmpl, mpdRepresentation{}, base, ast, epoch, 0, 0, true, false)
	if err != nil {
		t.Fatalf("expandTemplate: %v", err)
	}
	if len(segs) != 3 {

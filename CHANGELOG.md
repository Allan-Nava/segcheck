...
- **SEO for the published page** (SC-105). The crawler-facing half of the docs
  site, generated from the page rather than maintained beside it: `sitemap.xml`,
  `robots.txt`, and an `llms.txt` for the crawlers that read prose rather than
  markup — what segcheck is, every check and what it compares, the statuses, and
  what it is not. In the page: `robots`, `theme-color`, `og:locale` and
  `og:image:alt`, a `rel=sitemap` link, and the fields a search result is built
  ...
  one is why the gate exists at all — the first sitemap this change shipped
  listed `running-in-containers.html`, and Pages serves `docs/` as committed, so
  the `.md` answers 200 and the `.html` 404s. Seventeen fixture tests in
  `scripts/seo_test.sh`, one of them that exact case.

  Why it matters mechanically rather than as hygiene: crawlers read `robots.txt`
  only from the host root, and `allan-nava.github.io`'s is generated by a daily
  sync from the project sitemaps that answer **200**. A site that ships none is
  simply absent from it — segcheck was one of 28.

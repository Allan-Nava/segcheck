...
22 checks: 18 OK, 1 WARN, 3 BAD, 0 ERROR — 18 segments, 24.5 MiB in 4.1s
```
...
# Did it get *worse*? Save a run, compare a later one against it
...
`--baseline run.json` answers the question a single run cannot: not "is this bad" but "did this get worse". Give it a file `--output json` wrote earlier and the diff arrives as ordinary findings on a `baseline` check, so it sorts worst-first with everything else, renders in every format, and `--exit-on bad` gates on a regression without knowing it came from a comparison. What it compares is decided by what is actually stable between two runs, and most of a finding is not. A rendition is stable and one of its segments is not — a live stream has different segments every run, so `720p seg 38` is never `720p seg 41` — which is the same reason the metric renderers carry no target label. So a check is compared *per rendition*, folding in whatever it said about that rendition's segments: without that, a stream breaking would go unreported, because the rendition-level OK is replaced by a segment-level BAD and the pair looks deleted rather than worse. A measurement has to move more than 10% to count, since a measured number wobbles between runs; it is compared only against the same unit, because a check that changed bps to Mbps has not found a different stream. A message is compared only for a finding that carries no measurement at all — `resolution` says "coded 1280x720 matches the declared resolution" and nothing else, so there the message *is* the observation, and that is how a resolution that moved gets caught in the case where both runs agree with their own manifest and neither is a defect. A check that reported in the baseline and reports nothing now is an `ERROR`, not a `BAD`: the stream is not known to be broken, the coverage is known to have a hole. One caveat worth knowing: comparing two runs taken with different sampling flags reports the sampling change, not a stream change — drop `--renditions` from one and the rungs it stopped sampling are correctly reported as gone.
...
| `fetch` | Every sampled segment is reachable, and none was truncated by the byte cap | ERROR |
...
| `discontinuity` | `EXT-X-DISCONTINUITY` against the timeline reset it promises — a tag with nothing behind it is a decoder flush the viewer pays for and nobody asked for — and `EXT-X-DISCONTINUITY-SEQUENCE` across the ladder, so two rungs never put the same media on two different timelines | BAD |
...
| `byterange` | Whether the origin honours an HTTP `Range` request, measured once per host rather than once per segment: fatal to a stream addressed in byte ranges, free to one that is not, and a warning either way when `Accept-Ranges: bytes` claims support the origin does not have | BAD |
...
| `dvr` | The oldest segment the DVR window still promises — DASH `timeShiftBufferDepth`, or an HLS playlist's own span — is fetched and parsed, and when it is not there, four more probes bisect the window for how much of it the origin really holds. It is the only promise nobody collects on purpose | BAD |
...
| `watch` | With `--watch`, what the live edge actually does: new-segment latency, a stall, a packager that stopped publishing, an edge advancing every poll and still losing ground against real time, and one that moves backwards | BAD |
| `period` | A multi-period MPD read as one presentation: whether each Period's media lands where `@presentationTimeOffset` maps it, and whether the resolution or codec changes across a boundary. Every other check is scoped to a period too, so consecutive Periods are never compared as competing rungs of one ladder. A Period whose position nothing in the MPD determines is reported as a hole in the coverage, never placed at a guess | BAD |

A multi-period MPD is one presentation, not several. Each `Period` restarts its own media timeline, so nothing in a segment says which period it came from and a defect at a boundary is invisible from either side: every period checks out perfectly on its own and the join is the thing nobody looked at. Segment placement is arbitrated by the media — `@presentationTimeOffset` says which media time a period begins at, the template says how far into the period each segment starts, and the `tfdt` settles it — and the boundary is checked for the encoder change a new period is so often there to carry. A period's first segment is allowed to begin before the period does, because a segment grid does not divide by a boundary and the player trims the head; audio is left out of the placement reading entirely for the same reason, since an AAC grid straddles almost every boundary there is. Where a Period begins is itself derived rather than read: `@start` is optional, and a Period without one begins where the previous one ended, which is only knowable if that one stated a `@duration`. A Period held in another document behind an `xlink:href` states neither here, so the Period after it has no derivable position at all — segcheck says so and places nothing, because the alternative is printing the presentation's beginning for media five minutes into it.

`EXT-X-DISCONTINUITY` is an instruction, not a description: it tells the player to throw its decoder away and start again. `continuity` reads it as a licence — a jump here is expected rather than a defect — and the other direction is the half nobody looks at, because neither the manifest nor the media is individually wrong. A tag over a timeline that runs straight through it, on media that did not change shape either, is a flush and a re-initialisation performed for no reason; a packager emitting one per segment costs a hitch per segment. RFC 8216 is what keeps this honest: the tag signals a change of encoding as well as of timestamps, so a continuous timeline over media that really did change codec, track layout or size is the tag doing its job.

Whether an origin honours a `Range` request is a property of the host, not of a segment, and it is the one delivery fault that can break a stream outright while every byte of media on disk is perfect. A playlist packaged as byte ranges of one resource — HLS `EXT-X-BYTERANGE`, or DASH `SegmentBase` with an `indexRange`, which is the whole on-demand profile — does not degrade against an origin with range support off; it fails, and it fails wearing a media defect's clothes, because every segment a player asks for arrives as the entire file. The timeline then overlaps itself, the durations come out hundreds of percent long and the transport-stream continuity counters break at every join, so the one misconfiguration is reported as eight problems with the content. `byterange` names the cause once, for the host, above all of it. The other half is the familiar shape: `Accept-Ranges: bytes` is a claim and answering a Range request is the fact, so an origin that advertises support it does not have is warned about whatever this stream needs, because the next thing to read that header will believe it. What it is deliberately not is a defect everywhere it appears — a stream that addresses whole resources never sends a `Range` at all, so an origin ignoring one costs it nothing but a more expensive seek, and that is reported at OK. The probe is free on a stream already using byte ranges, since the sample itself asked the question, and costs one small request per host otherwise.

A live edge has to advance at 1× real time, which is not the same claim as "it advanced". A packager publishing two seconds of media every three moves every single time it is looked at and still loses a second of ground per three, so the live latency grows without bound until the viewer's buffer is gone and they rebuffer at a moment nothing in the stream explains. `--watch` measures the media published against the wall clock over the whole window, because no pair of polls can show it. The edge going backwards is the other shape a stall check reads as health — the newest segment changed, which is what a working edge does — and it is a packager that restarted or a POP answering with an older playlist.
